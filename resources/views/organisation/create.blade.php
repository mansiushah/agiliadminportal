...
<div class="right_bottom_ttl mb-4">
    <div class="right_bottom_ttl_lft d-flex ">
        <a href="{{route('organisations')}}" class="user_back_btn"><img src="{{url('public/assets/img/back-arrow.svg')}}" alt="back-arrow"
                class="img-fluid"></a>
        <h4 class="ml-3 mb-0">Add new organisation</h4>
    </div>
...
<div class="manage_user_main_card">
    <div class="manage_user_card_ttl">
        ...
        <p>Add a new organisation to the system</p>
    </div>
  ...
  @csrf
         <div class="form-row add_user_form_row">
            ...
            <div class="form-group col-md-6 mb-3">
                ...
                <input type="text" class="form-control" name="company_number" placeholder="Company number" value="{{ old('company_number') }}">
        ...
        <div class="form-row add_user_form_row">
            <div class="form-group col-md-12">
                ...
                <input class="form-control" name="trading_name" type="text" placeholder="Trading name" value="{{ old('trading_name') }}">
               	@error('trading_name')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
        ...
        <div class="form-row add_user_form_row">
            <div class="form-group col-md-12">
                ...
                <input class="form-control" name="company_email" type="text" placeholder="Company email" value="{{ old('company_email') }}">
              	 @error('company_email')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div class="form-group col-md-12">
                ...
                <!-- <div class="form-group col-md-12 px-0">
                    <gmp-place-autocomplete id="autocomplete"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd;color: #000; border-radius: 6px;"
                        placeholder="Enter address"></gmp-place-autocomplete>
                </div> -->
      ...
      <input type="hidden" name="company_address_google_placeid" id="place_id" placeholder="Google Place ID">
                ...
                <select class="form-select" aria-label="Default select example" name="usa_state_id">
                    <option value="">Select Usa state</option>
                      @foreach ($usastates as $key => $value)
                              <option value="{{ $value->usa_state_id }}" {{ old('usa_state_id') == $value->usa_state_id ? 'selected' : '' }}>
                ...
                <!-- <select name="currency_id" class="form-select" aria-label="Default select example">
                   <option value="">Select Currency</option>
                      @foreach ($currencies as $key => $value)
                              <option value="{{ $value->id }}" {{ old('currency_id') == $value->id ? 'selected' : '' }}>
        ...
        <div id="vatFieldsContainer" class="mt-3"></div>
            ...
            <div class="form-group col-md-12">
                ...
                <input type="text" class="form-control" placeholder="Enter VAT number" name="vat_number" value="{{ old('vat_number') }}">
            </div>
        </div> -->
                ...
                <select class="categories_select" name="category_id[]" multiple="multiple">
                    <option value="">Select category</option>
                      @foreach ($category as $key => $value)
                              <option value="{{ $value->id }}" {{ (is_array(old('category_id')) && in_array($value->id, old('category_id'))) ? 'selected' : '' }}>
                              {{ $value->name }}</option>
                      @endforeach
                </select>
                @error('category_id')
            ...
            <div class="form-group col-md-12 verification-radio add_verification-radio mb-2">
                <div class="form-check form-check-inline pl-0 mb-0">
                    ...
                    <div class="radio ml-0">
                        <input id="radio-5" name="offer_quota" type="radio" class="unlimited_ofr_inpt" value="0" {{ old('offer_quota', $offer->offer_quota ?? 0) == 0 ? 'checked' : '' }}>
                ...
                <div class="form-check form-check-inline pl-0">
                    <div class="radio ml-0">
                        <input id="radio-6" name="offer_quota" type="radio"
                            class="limited_ofr_inpt" value="1" {{ old('offer_quota', $offer->offer_quota ?? 0) == 1 ? 'checked' : '' }}>
                        <label for="radio-6" class="radio-label">Limited offers</label>
                    </div>
                </div>
            </div>
        </div>
        <div class="form-row add_user_form_row add_maximum_offer_input {{ old('offer_quota', $offer->offer_quota ?? 0) == 1 ? 'limited_ofr_inpts' : '' }}">
            <div class="form-group col-md-12 mb-3">
                <input type="number" class="form-control"
                    placeholder="Enter maximum number of offers" name="max_number_of_offer" value="{{ old('max_number_of_offer') }}">
            </div>
        ...
        <div class="form-row mt-2 organaization_button">
            ...
            <button type="button" class="btn btn-primary Cancle_outline_btn mb-3" onclick="window.location.href='{{ url('/organisations') }}'">Cancel</button>
        </div>
              </form>
    </div>
</div>
