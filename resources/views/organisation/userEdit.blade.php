...
<div class="right_bottom_ttl_lft d-flex ">
    <a href="{{ route('organisations.view',['id'=>$data->id])}}" class="user_back_btn"><img src="{{url('public/assets/img/back-arrow.svg')}}" alt="back-arrow"
            class="img-fluid"></a>
...
<div class="add_user_form mt-3">
    <form method="POST" action="{{ route('organisations.user.update') }}">
        @csrf
                ...
                <select name="time_zone_name" class="form-select" aria-label="Default select example">
                   <option value="">Select</option>
                      @foreach ($timezone as $key => $value)
                              <option value="{{ $value->Name }}" {{ $data->time_zone_name ==  $value->Name  ? 'selected' : '' }}>
                              {{ $value->Name }}</option>
                      @endforeach
                </select>
   ...
   <!--      <div class="form-row add_user_form_row add_offer_quatabx "> -->
      <div class="form-row add_user_form_row add_offer_quatabx {{ $data->role_id != 3 ? 'limited_ofr_inpts' : '' }}">
            <div class="form-group col-md-12 add_user_phone">
                <label>Offer quota <span class="optional_span">(optional)</span></label>
                <input type="text" class="form-control" placeholder="5" name="user_offer_quota" value="{{ $data->user_offer_quota }}">
            </div>
        </div>
